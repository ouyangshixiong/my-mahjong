...
- 支持四川麻将血战玩法：三门 108 张，发牌后四家先各选同一花色 3 张，按顺时针/逆时针/对家随机方向同时交换，再进行定缺。
...
- `POST /ai/exchange`：按起手牌返回换三张建议。
- `POST /ai/lack-suit`：按起手牌返回定缺建议。
- `POST /ai/discard`：按 `rulesetId` 返回 AI 出牌建议。
- `POST /ai/score`：按 `rulesetId` 算番。

`/ai/analyze`、`/ai/discard`、`/ai/score` 必须显式传入 `lackSuit`；四川血战传 `m`/`p`/`s`，无定缺玩法传 `null`。`/ai/discard` 还必须显式传入 `mustDiscard`：正常摸牌为 `false`，碰牌后必须出牌为 `true`。

## 规则依据与默认选项

实现按当前主流成都麻将/四川血战规则交叉核对，关键流程为“换三张在前、定缺在后”：
...
不同地区和平台对自摸加底/加番、呼叫转移、封顶番数、花猪赔付对象等存在差异。本规则集默认使用自摸加底、3 番封顶、花猪向未胡且非花猪者赔付、不开启呼叫转移；这些差异应通过规则集字段显式配置，不在代码中静默降级。
